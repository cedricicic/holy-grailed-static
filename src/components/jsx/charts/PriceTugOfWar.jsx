...
const PriceTugOfWar = ({ originalListing, relatedListings }) => {
  ...
  useEffect(() => {
    ...
    const x = d3.scaleLinear().domain([0, priceBands - 1]).range([0, innerWidth]);
    ...
    const nodeElements = svg
      ...
      .on('mouseover', (event, d) => {
        // Capture pointer coordinates relative to the SVG container
        const pointer = d3.pointer(event, svgRef.current);
        setHoveredNode({ ...d, pointer });
      })
    ...
    function addJitter() {
      simulation
        .force(
          'jitter',
          d3.forceX(d => x(d.bandIndex) + (Math.random() - 0.5) * 40).strength(0.1)
        )
        .alpha(0.3)
        .restart();
    ...
    function tugOfWar() {
      simulation.force('x', d3.forceX(d => x(d.bandIndex)).strength(0.9)).alpha(0.2).restart();

      setTimeout(() => {
        simulation.force('x', d3.forceX(d => x(d.bandIndex)).strength(0.7));
  ...
  return (
    <>
      ...
      >
        <svg ref={svgRef}></svg>
        {hoveredNode && hoveredNode.pointer && (
          <div
            style={{
              position: 'absolute',
              left: `${hoveredNode.pointer[0] + 10}px`,
              top: `${hoveredNode.pointer[1] - 10}px`,
          ...
          >
            <div>
              <strong>{hoveredNode.isOriginal ? 'Your Listing' : 'Similar Listing'}</strong>
            </div>
